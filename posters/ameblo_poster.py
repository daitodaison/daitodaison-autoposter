    ...
    async with async_playwright() as p:
        ...
        if image_path and os.path.exists(image_path):
            ...
            try:
                ...
                # --- C) 画像クリック → スキップ ---
                # 【実機検証済み 2026/06/27】
                # Amebloはアップロード完了後、モーダル内の左上（先頭）画像を
                # 自動的に選択済み状態（緑枠）にする仕様。
                # そのため「クリックして画像を選ぶ」操作は不要で、
                # li要素がvisibleでない（ローディング中）のにクリックして
                # Timeout 10000msエラーになっていたのが従来の失敗原因。
                # → クリック処理を完全に削除し、直接Dへ進む。
                await page.screenshot(path="ameblo_04c_skip_click.png")
                log.info("【STEP4-C】クリックスキップ（アップロード後の自動選択仕様を利用）→ STEP4-Dへ")
                ...
                # ⚠️ 【2026/06/27時点のAmeblo仕様注意】
                #    下書き保存を押すとカバー画像が消える不具合あり。
                #    公開（投稿する）ボタンでは画像は保持される。
                #    このスクショが「画像セット成功」の唯一の証拠になる。
