...
"""
article_writer.py
記事執筆ロジック: タイトル生成・セクション別生成・記事組み立て
GAS版の _タイトル生成 / _セクション生成 / _記事後処理 を完全移植
"""

import re
import logging
from ai_client import call_ai_with_fallback, call_ai_section
from templates import select_template, get_まとめ指示
from text_processing import post_process_article, detect_abstract_phrases, is_article_acceptable
from data_store import get_conversion_goal

log = logging.getLogger(__name__)

# ================================================================
# GAS版ライター設定（_セクション生成内の【禁止】ブロックを完全移植）
# ================================================================
WRITER_BASE_PROMPT = """
【禁止（最優先・絶対厳守）】
...
- 中国語・韓国語の漢字は絶対に使わないこと（例: 「市场」→「市場」「检討」→「検討」「收入」→「収入」）
- 架空の収益額・勝率・pipsは絶対に書かないこと（例: 「月収20万」「勝率70%」「月5万稼いだ」は禁止）
- 「合格した」「パスした」「チャレンジを達成した」という表現は使わないこと
- 「FX歴○年」「トレード歴○年」などの架空経歴は書かないこと
- 「絶対に稼げる」「誰でも簡単」「リスクゼロ」は使わないこと
- 数字を使う場合は Fintokei の実際のルール（日次損失・最大損失・ロット制限）のみ使用可
- 「お客様」は使わず「あなた」と書く

【書く人物 daito のプロフィール（厳守）】
- 製造現場（トヨタ系）・接客・音楽・輸出入・ライター・イラスト・小説・ゲーム開発など10以上の職を経験
...
【文体ルール（絶対）】
- 「ます。」「です。」調で統一
- 「だ。」「である。」「できた。」「わかった。」で終わる文は全て「です/ます」に直すこと
...
5. 中国語・韓国語の漢字は絶対に使わないこと

【禁止】
- 簡体字・繁体字（检討・収入の「收」・市场など）を使わないこと
- 架空の数字・成果を含めないこと
...
タイトルを1行だけ出力してください（説明・コメント不要）:"""
    ...
    log.warning(f"⚠️ タイトル生成失敗 → フォールバックタイトル使用")
    return f"{kw} ｜ 正直な現場目線でまとめました"


def generate_section(kw: str, section_name: str, instruction: str,
                     min_chars: int, prev_text: str,
                     max_chars: int = None, emotion: str = "") -> str:
    """
    GAS版 _セクション生成 の完全移植
    禁止ルール・文体ルール・リトライロジックをすべて再現
    """
    if max_chars is None:
        max_chars = int(min_chars * 1.2)

    # 直前テキストから禁止フレーズを抽出（GAS版の _前文から禁止フレーズを抽出 に相当）
...
def generate_article(kw: str, trend_info: str = "", target_chars: int = 1800) -> dict:
    """
    記事を1本生成して辞書で返す
    {title, body, quality_score, passed}
    """
    log.info(f"***執筆中*** {kw}")
    log.info(f"  >> 目標文字数: {target_chars}字")

    # タイトル生成
    title = generate_title(kw)

    # テンプレート選択（GAS版 _記事テンプレートを選択 に相当）
    template = select_template(kw)
    log.info(f"  >> 記事テンプレート: {template['name']}（スコア:{template['score']}）")

    sections = template["sections"](kw, target_chars)
    body = title + "\n\n"

    # セクション生成ループ
    for i, section in enumerate(sections):
        sec_name = section["name"]
        min_chars = int(target_chars * section["ratio"])
        max_chars = int(min_chars * 1.2)
        instruction = section["instruction"]
        emotion = section.get("emotion", "")

        log.info(f"  >> {sec_name}（最低{min_chars}字）")
        sec_text = generate_section(kw, sec_name, instruction, min_chars, body, max_chars, emotion)
        body += sec_text + "\n\n"
    ...
    # ゴール判定・誘導文挿入（GAS版 _記事後処理 に相当）
    goal = get_conversion_goal(kw)
    log.info(f"  >> ゴール: ***{goal['key']}*** score={goal['score']} → {goal['url']}")

    # 抽象表現チェック・具体化（GAS版 _具体化リライト に相当）
    ...
    if passed:
        ...
        log.warning(f"  >> ⚠️ 品質基準未達: {quality['issues']}")
...
def _rewrite_abstract(text: str, kw: str) -> str:
    """
    GAS版 _具体化リライト / _抽象表現を具体化 の移植
    抽象表現を具体的な数字・手順・実体験に置き換える
    """
    prompt = f"""あなたはプロの編集者です。以下の文章に含まれる抽象的な表現を、
読者がすぐに行動に移せるような「具体的な数字・手順・daito（製造現場出身）の実体験に基づいた思考」に置き換えて書き直してください。

【リライトの鉄則】
1. 「重要です」「意識してください」を「私は〇〇しています」「具体的には△△をチェックします」に変える
2. 可能な限り、具体的な数字（時間、回数、設定値など）を出す（架空の収益・勝率は禁止）
3. daitoの「現場の規律」というトーンを維持する
4. 中国語・韓国語の漢字（检討・收入・市场など）は絶対に使わないこと
5. メタ発言（「リライトしました」等）は一切不要。本文のみ出力
